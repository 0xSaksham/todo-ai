...
export const completedTodos = action({
  args: {},
  handler: async (ctx): Promise<any[]> => {
    const userId = await handleUserId(ctx);
    if (userId) {
      const todos: any[] = await ctx.runQuery(
        api.todos._internal_completedTodos,
        {}
      );
      return todos;
    }
...
export const _internal_completedTodos = query({
  args: {},
  handler: async (ctx) => {
    const userId = await handleUserId(ctx);
    if (userId) {
...
export const inCompleteTodos = action({
  args: {},
  handler: async (ctx): Promise<any[]> => {
    const userId = await handleUserId(ctx);
    if (userId) {
      const todos: any[] = await ctx.runQuery(
        api.todos._internal_inCompleteTodos,
        {}
      );
      return todos;
    }
...
export const _internal_inCompleteTodos = query({
  args: {},
  handler: async (ctx) => {
    const userId = await handleUserId(ctx);
    if (userId) {
...
export const totalTodos = action({
  args: {},
